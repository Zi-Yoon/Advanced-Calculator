...
// State
localparam  ST_INIT     = 3'b000, // INIT
            ST_NUM_A    = 3'b001, // NUM A
            ST_OPER     = 3'b010, // OPERATION
            ST_NUM_B    = 3'b011, // NUM B
            ST_RESULT   = 3'b100, // CALCULATE
            ST_KEEP_CAL = 3'b101, // NEXT CALCULATE
            ST_ERROR    = 3'b110; // ERROR
...
reg [ 2:0] r_bias_a      ; // A Dot Location
reg [ 2:0] r_bias_b      ; // B Dot Location
reg [ 2:0] r_bias_result ; // Calculating Dot Location
...
always @(posedge i_clk or negedge i_rstn) begin
    if (!i_rstn) begin
        r_key_valid_now  <= 1'b0 ;
        r_key_valid_next <= 1'b0 ;
    end
    else begin
        r_key_valid_now  <= i_key_valid     ;
        r_key_valid_next <= r_key_valid_now ;
    end
end
*/
    ...
    if (!i_rstn) begin
        ...
        // Number B - One Digit Moving Left
            ...
            // Make Result - Oper or Ent
                ...
                r_dot_valid <= 1'b1 ;
                case (r_oper_a)
                    0 : r_result <= (w_num_a / w_bias_a) / (w_num_b / w_bias_b) ; // /
                    1 : r_result <= (w_num_a / w_bias_a) * (w_num_b / w_bias_b) ; // *
                    2 : r_result <=           w_ex_num_a - w_ex_num_b           ; // -
                    3 : r_result <=           w_ex_num_a + w_ex_num_b           ; // +
                    default : r_result <= 0 ;
                endcase
                if (i_type == TYPE_OPER) begin
                    r_oper_b <= i_bcd_data[1:0] ;
                end
            end
        end
        // Calculate
        else if (r_state == ST_KEEP_CAL) begin
            r_bcd_a  <= r_bcd_result ;
            r_bias_a <= r_bias_result ;
            r_oper_a <= r_oper_b;
        end
        // Error
...
always @(posedge i_clk or negedge i_rstn) begin
    ...
    else if (r_result > 99999999) begin
        o_err <= 1'b1;
    end
    // Underflow
...
// Bias A
assign w_bias_a =   (r_bias_a == 7) ? 10000000 : 
                    (r_bias_a == 6) ? 1000000  : 
                    (r_bias_a == 5) ? 100000   : 
                    (r_bias_a == 4) ? 10000    : 
                    (r_bias_a == 3) ? 1000     : 
...
// Bias B
assign w_bias_b =   (r_bias_b == 7) ? 10000000 : 
                    (r_bias_b == 6) ? 1000000  : 
                    (r_bias_b == 5) ? 100000   : 
                    (r_bias_b == 4) ? 10000    : 
                    (r_bias_b == 3) ? 1000     : 
...
// Expanded Number A
assign w_ex_num_a = w_num_a * 100000000 / w_bias_a ;

// Expanded Number B
assign w_ex_num_b = w_num_b * 100000000 / w_bias_b ;
...
// Display Digit
assign w_seg_data = (r_state == ST_INIT    ) ? 35'd0                         :
                    (r_state == ST_NUM_A   ) ? {r_bias_a     , r_bcd_a     } :
                    (r_state == ST_NUM_B   ) ? {r_bias_b     , r_bcd_b     } :
                    (r_state == ST_KEEP_CAL) ? {r_bias_a     , r_bcd_a     } : 
                    (r_state == ST_RESULT  ) ? {r_bias_result, r_bcd_result} : 35'd0 ;

// Output
assign o_seg_data = w_seg_data ;

// Operation LED
assign o_led_op =   (r_oper_a == 2'd0 && r_state == ST_OPER) ? 4'b1110 :
                    (r_oper_a == 2'd1 && r_state == ST_OPER) ? 4'b1101 :
                    (r_oper_a == 2'd2 && r_state == ST_OPER) ? 4'b1011 :
                    (r_oper_a == 2'd3 && r_state == ST_OPER) ? 4'b0111 : 4'b1111 ;
